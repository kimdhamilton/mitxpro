...
function configureHlsVideo(selector, autoplay) {
  ...
  if (video) {
    ...
    // Note: it would be more normal to wait on the 'canplay' event below however on Safari (where you are most likely to find built-in HLS support) the user must explicitly enable autoplay for the video.src domain
    // before a 'canplay' event will be emitted; the last video event that can be reliably listened-for when autoplay is not allowed on the domain is 'loadedmetadata'.
